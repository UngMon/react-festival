...
import { Count } from "../../../type/UserDataType";
...
const Feelings = React.memo(({ collectionName, contentId }: T) => {
  ...
  useEffect(() => {
    if (!loading || firebaseState.loadingState === "pending") return;

    const getFeelingData = async () => {
      const countRef = doc(db, collectionName, contentId, "feeling", "count");
    ...
    getFeelingData();
  }, [collectionName, contentId, loading, firebaseState]);

  const handler = async (type: string) => {
    if (!firebaseState.loginedUser || firebaseState.userUid === "")
    ...
    let newDataObject: { [key: string]: FieldValue } = {};
        ...
        <div className="feel-box" onClick={() => handler("like")}>
          ...
          {!loading && (
            <p
              style={{
                color:
                  userPick === "like" && firebaseState.loginedUser ? "red" : "",
              }}
        ...
        <div className="feel-box" onClick={() => handler("dislike")}>
          ...
          {!loading && (
            <p
              style={{
                color:
                  userPick === "dislike" && firebaseState.loginedUser
                    ? "red"
                    : "",
              }}
              className="feeling-count"
