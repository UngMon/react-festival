...
import { ContentIdCode } from "constant/catCode";
import "./Input.css";
...
const Input = ({ params, setPage }: T) => {
  ...
  const submitHandler = (e: FormEvent) => {
    ...
    navigate(`/search?contentTypeId=${contentTypeId}&keyword=${text}&page=1`);
  };

  const listClickHandler = (tpye_num: string) => {
    setContentType(tpye_num);
    setPage(1);
    navigate(`/search?keyword=${text}&contentTypeId=${tpye_num}&page=1`);
  };
